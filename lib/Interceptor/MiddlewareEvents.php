...
use \Telcos\MX\Client\Model\Errors;
use \Telcos\MX\Client\Model\Error;

class MiddlewareEvents
{
    
    function __construct(\Telcos\MX\Client\Interceptor\KeyHandler $signer)
    {
        $this->signer = $signer;
...
function build_error($code, $message){
    $error = new \Telcos\MX\Client\Model\Error([
        "code" => $code,
        "message" => $message
    ]);
    $errors = new \Telcos\MX\Client\Model\Errores(["errors" => [$error]]);
