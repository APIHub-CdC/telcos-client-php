...
use \Telcos\Client\Interceptor\KeyHandler;
use \Telcos\Client\Interceptor\MyLogger;
use \Telcos\Client\Model\Errors;
use \Telcos\Client\Model\Error;

class MiddlewareEvents
{
    
    function __construct(\Telcos\Client\Interceptor\KeyHandler $signer)
    {
        $this->signer = $signer;
...
function build_error($code, $message){
    $error = new \Telcos\Client\Model\Error([
        "code" => $code,
        "message" => $message
    ]);
    $errors = new \Telcos\Client\Model\Errores(["errors" => [$error]]);
